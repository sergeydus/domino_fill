...
> **Done in row 20b.** The page said `Create Next App` / `Generated by create next app`
> until now — the browser tab, the bookmark, the link preview in a chat app and the search
> result all carried it. For a game whose entire retention model is "come back tomorrow",
> being unnamed in every one of those places is not cosmetic.
>
> `metadataBase` is set from the environment, because Open Graph URLs must be absolute:
> most crawlers do not resolve a relative `og:image`, so a relative one is the same as no
> image and fails invisibly — the page itself renders perfectly either way. The browser
> test asserts the served tag starts with a scheme rather than trusting the config.
>
> **The icons are generated, not pasted.** `scripts/icon.ts` draws the mark — a one-pip
> upright domino in the board's own palette — and renders it at 192, 512 and 180 through a
> small hand-written PNG encoder; `npm run icons` writes them. A checked-in binary is the
> one file nobody reviews: it cannot be read in a diff, and nothing tells you whether the
> 512 and the 180 are even the same picture. `tests/icons.test.ts` holds the committed
> bytes to what the generator produces, checks the encoder's own header, and samples the
> same three relative points at two sizes to prove it is one drawing scaled rather than
> three drawings.
>
> The manifest is `app/manifest.ts` rather than a static file, so its icon paths and name
> cannot drift from the app's metadata without TypeScript noticing, and it declares a
> `maskable` icon separately from the `any` ones — Android crops maskable icons to the
> launcher's shape, and a missing declaration gets a white box with the icon shrunk inside
> it. `display: 'standalone'`, not `fullscreen`: hiding the status bar takes the clock away
> from someone playing a *daily* puzzle.
>
> The browser test fetches every icon the manifest names and requires a 200 and an
> `image/png`. That is the failure this item is really about: a manifest is valid JSON
> whether or not its icons resolve, and Chrome responds to a broken one by silently
> declining to offer installation.
>
> `userScalable` is left **unset** rather than set to `true`, and the test asserts the
> *absence* of `user-scalable=no` and of a pinned `maximum-scale`, because scalable is the
> default and the contract is that nothing bans it.
