...
const strokeWidth = 6
const Rock: React.FC<React.SVGProps<SVGSVGElement>> = (props) => {
    const { boardsStore } = useStores()
    const size = boardsStore.squareSize
    return (
        <svg width={size} height={size + 16} {...props} className="translate-y-[-16px]">
            ...
            <rect x={strokeWidth} y={strokeWidth} width={size - (strokeWidth * 2)} height={size - (strokeWidth * 2) + 16} fill="transparent" strokeWidth={6} stroke="black" rx={8} ry={8} />
        </svg>
    );
};
